...
Dopo aver effettuato il login a `r config$remote$login`, `which` dovrebbe essere in grado di trovare `amdahl` senza difficoltà. Se si è dovuto caricare un modulo Python, caricarlo di nuovo.
...
Come in precedenza, utilizzare i comandi di stato `r config$sched$name` per verificare se il lavoro è in esecuzione e quando termina:
